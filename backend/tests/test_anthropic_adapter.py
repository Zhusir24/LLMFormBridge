...
class TestAnthropicAdapter:
    ...
    def adapter(self):
        """创建Anthropic适配器实例"""
        return AnthropicAdapter(api_key="test_api_key")
    ...
    def test_custom_api_url(self):
        ...
        adapter = AnthropicAdapter(
            ...
            api_url="https://custom.anthropic.com/v1"
        )
        assert adapter.api_url == "https://custom.anthropic.com/v1"

    def test_default_api_url(self, adapter):
        """测试默认API URL"""
        assert adapter.get_default_api_url() == "https://api.anthropic.com/v1"
    ...
    def test_transform_request_to_anthropic_without_system(self, adapter):
        ...
        assert anthropic_request["model"] == "claude-3-haiku-20240307"
    ...
    def test_transform_request_to_openai_without_system(self, adapter):
        ...
        assert openai_request["model"] == "claude-3-haiku-20240307"  # 保持原始模型名
        assert len(openai_request["messages"]) == 1
        assert openai_request["messages"][0]["role"] == "user"
    ...
    def test_transform_response_from_openai(self, adapter):
        ...
        assert len(llm_response.choices) == 1
        assert llm_response.choices[0]["message"]["content"] == "Hello! How can I help you?"
        assert llm_response.usage["prompt_tokens"] == 10
    ...
    def test_model_mapping_to_openai(self, adapter):
        ...
        assert adapter._map_model_to_openai("claude-sonnet-4-20250514") == "gpt-4o"
